...
export function PromotorEditar() {
  ...
  const promotor = promotoresData.find(p => p.id === promotorId);
  ...
  useEffect(() => {
    if (promotor) {
      setFormData({
        ...
        cpf: promotor.cpf,
        rg: promotor.rg,
        dataNascimento: promotor.dataNascimento,
        ...
        cep: promotor.cep,
        cargo: promotor.cargo,
        status: promotor.status,
        comissao: promotor.comissao,
        observacoes: promotor .observacoes,
        territorios: promotor.territorios,
        representadas: promotor.representadas
      });
    }
  }, [promotor]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/promotores/${promotorId}`);
    } catch (err) {
      setError('Erro ao atualizar o promotor. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex-1 space-y-4 p-8 pt-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-3xl font-bold tracking-tight">Editar Promotor</h2>
        </div>
      </div>
